...
// SumAreas masahataro baham jam mikone ke ye shapes i ro migire ke az noe intere.
// total:=0.0 yani az ghabl sefre.hamontori ke midonim halghe faghat.
// tooye araye toria mitone iterate bokone. vase hamin asan too function esh shapes ro
// be onvane vorodi be shekle araye tarif kardim ke khob albate noesh az type e intere
// ke inter ham ye class jodagoonast ke az noe interface e. harfesh ine ke age jai mikhay
...
type RegularUserAction struct {
	User *User
}

func (rua *RegularUserAction) PerformAction() string {
	return fmt.Sprintf("this is regular user's access for %s", rua.User.Username)
}
...
func (aa *AdminAction) PerformAction() string {
	return fmt.Sprintf("this is admin user acces  %s and the access level is %s ", aa.Admin.Username, aa.Admin.AccessLevel)
}
...
func forprocesscode() {
	...
	admin := &Admin{Username: "admin_user", AccessLevel: "3"}

	// Creating instances of RegularUserAction and AdminAction
	regularUserAction := &RegularUserAction{User: user}
	adminAction := &AdminAction{Admin: admin}

	// Processing and printing the results of different actions
	ProcessActions([]Action{regularUserAction, adminAction})
}
